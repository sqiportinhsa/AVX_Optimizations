...
Параллельно мы вычисляем значения для 8 соседних точек. При этом, в отличие от предыдущей части работы, здесь нет цикла, избыточно повторяющегося для части точек, что позволяет сделать предположение о примерном значении ускорения векторной реализации относительно скалярной в 8 раз.
...
Почему так вышло, что полученное ускорение в $9.5$ раза превысило ожидаемое ускорение в $8$ раз за счет использования SIMD оптимизаций? Посмотрим, что происходит с кодом при оптимизациях компилятора:
